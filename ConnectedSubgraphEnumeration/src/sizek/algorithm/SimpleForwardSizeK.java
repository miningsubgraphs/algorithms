 ...
 * @description: The algorithm works in a bottom-up way. This version is implemented mainly using ArrayLis, and BitSet (or boolean array) is used to ensure that the check whether a vertex is in the set in O(1) time.
...
public class SimpleForwardSizeK extends SubgraphEnumerationAlgorithm{
	
	private ArrayList<Integer> temp = new ArrayList();

	public SimpleForwardSizeK(ArrayList<Integer>[] g, ArrayList<ArrayList<Integer>> cs, int cardinality, int s, long maximalTime,
			String path) {
		graph = g;
		components = cs;
		k = cardinality;
	...
	public  boolean extendSubgraph(ArrayList<Integer> subgraph,  LinkedList<Integer> ex_neighbors, ArrayList<Integer> neighbors_subgraph, BitSet neighbors_subgraph_bitset, int forbiddenVerticesNumber, int max_index) {
		...
		while(ex_neighbors.size()>0) {
			...
			Integer i = ex_neighbors.getFirst();
			// remove the first vertex from the list
			ex_neighbors.removeFirst();
		...
		for(int i=fs; i>tp; i--) {
		    ...
		    temp.remove(i);
		    ex_neighbors.addFirst(deletedNeighbor);
		}	
		return hasInLeaf;
	}
}
