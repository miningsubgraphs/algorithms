...
public class SimpleForwardSizeKUsingHashSet extends SubgraphEnumerationAlgorithmUsingHashSet{

	public SimpleForwardSizeKUsingHashSet(HashSet<Integer>[] g, ArrayList<HashSet<Integer>> cs, int cardinality, int s, long maximalTime,
			String path) {
		graph = g;
		components = cs;
		k = cardinality;
	...
	public  boolean extendSubgraph(HashSet<Integer> subgraph,  LinkedList<Integer> ex_neighbors_list, HashSet<Integer> neighbors_subgraph, int forbiddenVerticesNumber, int max_index) {
		...
		while(iter.hasNext()) {
			// choose the first vertex from the list
			int i = ex_neighbors_list.getFirst();
			// remove the first vertex from the list
			ex_neighbors_list.removeFirst();
			// update the subgraph
			subgraph.add(i);
