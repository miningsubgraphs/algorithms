 ...
 * @description: The algorithm works in a top-down way. This version is implemented mainly using ArrayList.
 *               The graph is stored in ArrayList.
 *               Note that, only in this version, O(n+m) space is fully guaranteed.
 */
public class TopDownEnumerationSizeKUsingArrayList extends SubgraphEnumerationAlgorithmUsingArrayList {
	...
	public TopDownEnumerationSizeKUsingArrayList(ArrayList<Integer>[] g, ArrayList<ArrayList<Integer>> c,
	...
	public void enumerate() {
		...
		while (i.hasNext()) {
			...
			BitSet nonArticulationPoints = new BitSet();
			boolean[] mustIncludedVerticesBooleanArray = new boolean[size];
			// the subgraph
			BitSet subgraph = new BitSet();
			// the complementary set of the subgraph
			BitSet complementary = new BitSet();
			complementary.set(0, size);
			// initialize the adjacent relationship for each vertex of the component
			for (Integer j : component) {
				subgraphWithEdges[j] = ArrayListOpt.intersect(graph[j], component);
				nonArticulationPoints.set(j, true);
				subgraph.set(j, true);
				complementary.set(j, false);
			}
			findNonArticulationPoints(subgraphWithEdges, subgraph, nonArticulationPoints, A, B);
			topDownEnumeration(subgraphWithEdges, subgraph, complementary, nonArticulationPoints, A, B, mustInculdedVertices,mustIncludedVerticesBooleanArray);
		}
		if (pw != null)
			pw.close();
	}

	public void topDownEnumeration(ArrayList<Integer>[] subgraphWithEdges, BitSet subgraph, BitSet complementary, BitSet nonArticulationPoints,
			ArrayList<Integer> A, ArrayList<Integer> B, ArrayList<Integer> mustInculdedVertices, boolean[] mustIncludedVerticesBooleanArray) {
		...
		for (Integer i = nonArticulationPoints.nextSetBit(0); i >= 0; i = nonArticulationPoints.nextSetBit(i + 1)) {
			if (!mustIncludedVerticesBooleanArray[i]) {
				// update the subgraph
				subgraph.set(i, false);
				complementary.set(i, true);
				...
				int pb = B.size() - 1;
				// delete i from nonArticulationPoints
				nonArticulationPoints.set(i, false);
				if(subgraph.cardinality()>k)
				        findNonArticulationPoints(subgraphWithEdges, subgraph, nonArticulationPoints, A, B);
				topDownEnumeration(subgraphWithEdges, subgraph, complementary,  nonArticulationPoints, A, B, mustInculdedVertices,mustIncludedVerticesBooleanArray);
				// restore the subgraph
				subgraph.set(i, true);
				complementary.set(i, false);
				// restore the adjacent list representation of the subgraph
				if(subgraph.cardinality()>k+1)
				  for (Integer j : subgraphWithEdges[i]) {
					subgraphWithEdges[j].add(i);
				  }
				// restore the nonArticulationPoints, A and B
				nonArticulationPoints.set(i, true);
				int j = A.size() - 1;
				while (j > pa) {
					nonArticulationPoints.set(A.get(j), true);
					A.remove(j);
					j--;
				}
				int l = B.size() - 1;
				while (l > pb) {
					nonArticulationPoints.set(B.get(l), false);
					B.remove(l);
					l--;
				}
				// add vertex i to mustIncludedVertices
				mustInculdedVertices.add(i);
				mustIncludedVerticesBooleanArray[i]=true;
		...
		for (int i = mustInculdedVertices.size() - 1; i > pm; i--) {
			mustIncludedVerticesBooleanArray[mustInculdedVertices.remove(i)]= false;
		}
	...
	public void traverse(ArrayList<Integer> visited, ArrayList<Integer> subgraph, Integer index) {
		visited.add(index);
		ArrayList<Integer> adjacentVertices = ArrayListOpt.intersect(graph[index], subgraph);
