...
public class SimpleSizeKUsingBitSet extends SubgraphEnumerationAlgorithmUsingBitSet{

	public SimpleSizeKUsingBitSet(BitSet[] g, ArrayList<BitSet> c, int cardinality, int s, long maximalTime,
			String path) {
		graph = g;
		components = c;
		k = cardinality;
	...
	public  boolean extendSubgraph(BitSet subgraph, BitSet ex_neighbors, BitSet neighbors_subgraph, int forbiddenVerticesNumber, int max_index) {
		...
		for (int i = ex_neighbors.nextSetBit(0); i >= 0; i = ex_neighbors.nextSetBit(i+1)) {
			// update the subgraph
			subgraph.set(i, true);
